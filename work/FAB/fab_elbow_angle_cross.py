...
class fab_elbow_angle(object):
  ...
  def __init__(self,
    ...
    vh_start = self.pdb_var_H.atoms()[0].xyz
    vh_end = self.pdb_var_H.atoms()[-1].xyz
    ch_start = self.pdb_const_H.atoms()[0].xyz
    ch_end = self.pdb_const_H.atoms()[-1].xyz
    #
    mid_start = self.norm_vec(start=vh_end,end=vh_start)
    mid_end = self.norm_vec(start=ch_start,end=ch_end)
    ...
    # make sure the eigenvectors are pointing out from the midpoint
    eigenvectors_c = self.vector_dir(vec=eigenvectors_c,ref=mid_end)
    eigenvectors_v = self.vector_dir(vec=eigenvectors_v,ref=mid_start)
  ...
  def vector_dir(self,vec,ref):
    '''make sure the vector vec is in the direction of vector ref'''
    if vec.dot(ref) < 0:
      vec = -vec
    return vec
