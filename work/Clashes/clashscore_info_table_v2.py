...
class data_table_plot(object):
  """"""
  #----------------------------------------------------------------------
  def __init__(self,delta=10,delta_solvent=10,ratio=0.3,year_limit=2000,smaple_size=500):
    """Constructor"""
    self.delta = delta
    self.delta_solvent = delta_solvent
    self.ratio = ratio
    self.year_limit = year_limit
    self.smaple_size = smaple_size
    print 'nb_simple is nonbonded clashscore without sym op and solvent-solvent clashes'
    print 'clashscore is PROBE clashscore'
    print 'data for pdb file with no unknown pairs data, from the year {} and on'.format(self.year_limit)

  def get_data(self):
    ''' () -> dict,dict
    Read results of clash score survey for PROBE clashscore and restraints manager nonbonded clashscore
    ...
    self.clashscore_data_dict = pickle.load(open(os.path.join(datapath,'clashscore_data_dict'),'r'))

    experiment_dict_file = 'experiment_type_to_files_dict'		# source for experiment_type_dict
    self.experiment_type_dict = pickle.load(open(os.path.join(datapath,experiment_dict_file),'r'))
  ...
  def print_table(self,
                  ...
                  i_rec,j_rec,
                  test_type,test_val,
                  process_single_experiment = None,
                  print_sample_experiment_type=[]):
    '''
    rec number:
    ...
    The table will print comparison of two data sets from the "rec number" list above

    Arguments:
    i_rec: A number from the "rec number" list above
    j_rec: A number from the "rec number" list above
    test_type: 'delta' for absolute value of delta
               'signed_delta' without abs()
               'None'
               'ratio'
    test_val: the delta between the i_rec and the j_rec corresponding clashscores
    process_single_experiment: list of experiment types to be processed
    ...
    process_single_experiment = ['X-RAY DIFFRACTION']
    if not process_single_experiment:
      process_single_experiment = self.experiment_type_dict
    for experiment_type in process_single_experiment:
      # Collect all records from a particular experiment type, with clashsocre data, in the year range
      ...
      i = 0
      for x in records:
        if self.test(val1=self.clashscore_data_dict[x][i_rec],
...
def run():
  ...
  #                   test_val=table.delta,print_sample_experiment_type=[])
  #                   # test_val=table.delta,print_sample_experiment_type=['X-RAY DIFFRACTION'])
  # #
  # title = 'The number of files with (total nb_clashscore - clashscore (O_vdw 1.4)) > {}: '.format(table.delta)
  # table.print_table(title=title, test_type='signed_delta',i_rec=0,j_rec=4,
  #                   test_val=table.delta,print_sample_experiment_type=[])
  #
  ...
  table.print_table(title=title, test_type='signed_delta',i_rec=4,j_rec=1,
                    test_val=table.delta,print_sample_experiment_type=[])
  # #
  # title = 'The number of files with (nb_simple - clashscore (O_vdw 1.4)) > {}: '.format(table.delta)
  # table.print_table(title=title, test_type='signed_delta',i_rec=1,j_rec=4,
  #                   test_val=table.delta,print_sample_experiment_type=[])
  # #
  ...
  #                   test_val=table.delta,print_sample_experiment_type=[])
  #
  title = 'The number of files with (clashscore (O_vdw 1.52) - nb_simple) > {}: '.format(table.delta)
  table.print_table(title=title, test_type='signed_delta',i_rec=5,j_rec=1,
                    test_val=table.delta,print_sample_experiment_type=[])
  # #
  ...
  table.print_table(title=title, test_type='None',i_rec=2,j_rec=2,
                    test_val=table.delta_solvent,print_sample_experiment_type=['X-RAY DIFFRACTION'])
   #
  title = 'The number of files with symetry clashscore > {}: '.format(table.delta_solvent)
  table.print_table(title=title, test_type='None',i_rec=2,j_rec=2,
                    test_val=table.delta_solvent,print_sample_experiment_type=['X-RAY DIFFRACTION'])
