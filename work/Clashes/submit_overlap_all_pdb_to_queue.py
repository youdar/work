...
from nonbonded_pdb_data_collect import get_pdb_id
from libtbx.command_line import easy_qsub
import os

"""
Collect non-bonded overlap information from all PDB files that:
- have a single model
- have no unknown atom pairs
- have good cryst1 records
"""
...
class run_queue_tests(object):
  ...
  def __init__(self, clean_source=False,size_of_chunks=1):
    ...
    command = 'nonbonded_pdb_data_collect.py'
  ...
  def get_pdb_files(self):
    """ Get all pdb files form LBL mirror index  """
  ...
  def get_commands(self):
    ...
    for fn in self.pdb_file_with_path:
      ...
      outString = '{0} {1} >& log_{1}'.format(self.com_path,pdb_id)
